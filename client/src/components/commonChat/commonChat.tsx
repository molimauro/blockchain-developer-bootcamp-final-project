...
function Message({ id, body, from, readAt, sent }: DecryptedInbox) {
    const color =
        COLORS[addressToNumber(toHex(from.slice(-5))) % COLORS.length];
            ...
            if (friendPubKey?.[1]) {
                ...
                inboxListener = users.watchInbox(
                    ...
                    async (reply?: MailboxEvent, err?: Error) => {
                        ...
                        const message = await messageDecoder(
                            identity,
                            reply.message,
                        );
                ...
                <>
                    ...
                    <Flex alignItems="center">
                        <Input
                            ...
                            placeholder="Type some amazing content"
                            onKeyPress={e => {
                                if (e.key === "Enter") {
                                    handleSendMessage();
                                }
